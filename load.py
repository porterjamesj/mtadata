...
def save_to_db(db_sess, response_data):
    stop_times = []
    trips = []
    stops = []
    for st in response_data:
        ...
        if len(trip["route"]) > 1:
            if len(trip["route"]) == 2 and (trip["route"][-1] in ["N", "S"]):
                ...
                # TODO log warning of some kind or flag data as suspect
                pass
        ...
        assert stop_time["direction"] == st["stop"]["id"][-1]
...
def test_scrape():
    _, Session = start_db()
    scrape_date(parse_date("2018-05-04"), Session(), requests_session())
